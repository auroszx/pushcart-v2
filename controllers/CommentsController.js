...
module.exports = {
	...
	getCommentsByProduct: async function(product_id, token) {
		if (token != undefined) {
			db.connect();
			var comments = await db.get("SELECT product_comment_id, product_comments.user_id, product_comment FROM product_comments JOIN users ON users.user_id = product_comments.user_id WHERE product_id = (?)", [product_id]);
	...
	getCommentById: async function(product_comment_id, token) {
		if (token != undefined) {
			db.connect();
			return db.get("SELECT product_comment_id, product_comments.user_id, product_comment FROM product_comments JOIN users ON users.user_id = product_comments.user_id WHERE product_id = (?)", [product_comment_id]);
		}
		else {
			return { status: 403, message: "You are not allowed to perform this action" };
		}
